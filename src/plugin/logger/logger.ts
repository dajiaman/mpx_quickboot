...
const log: WechatMiniprogram.RealtimeLogManager | null =
  wx && wx.getRealtimeLogManager ? wx.getRealtimeLogManager() : null
const logger: WechatMiniprogram.LogManager | null =
  wx && wx.getLogManager ? wx.getLogManager({ level: 1 }) : null

type Key = 'log' | 'info' | 'warn' | 'debug' | 'error';

const idNoReg = /[\dxX]{10,}/g
// eslint-disable-next-line no-useless-escape
const nameReg = /^(([a-zA-Z+\.?\·?a-zA-Z+]{2,30}$)|([\u4e00-\u9fa5+\·?\u4e00-\u9fa5+]{2,30}$))/g
const phoneReg = /\d{11}/g
const emailReg = /[a-z0-9](\w|\.|-)*@([a-z0-9]+-?[a-z0-9]+\.|[a-z0-9]+\.){1,3}[a-z]{2,4}/gi

const logFn = (e: IArguments, key: Key, autoConsole: boolean) => {
  try {
    if (e.length > 0) {
      if (e.length === 1 && !e[0]) {
        throw new Error('请校验参数')
      } else {
        // 处理参数
 ...
 * 4、email
 */
const tranDataByRules = (data: string) => {
 ...
 */
const transferSensitiveInfo = (obj: any) => {
  try {
 ...
 * @returns
 */
const checkSensitiveInfo = (data: string) => {
 ...
 */
const processParams = function (params: any) {
  try {
    // 获取对象keys
